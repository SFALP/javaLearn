...
public class Q03_FibonacciListNumber {
    public static void main(String[] args) {
        ...
        if (sayi <= 1) {
            System.out.println("Daha buyuk sayi tanimlayiniz.");
        } else {

            while (fibonacci.get(i) < sayi) {//fibonacci elemanlari sayidan kucuk oldugu surece
                fibonacci.add(fibonacci.get(i) + fibonacci.get(i - 1));//bir onceki elemanla topla
                i++;
            }
        }
        if (fibonacci.get(fibonacci.size() - 1) > sayi) {//fibonaccideki son eleman, sayidan buyukse sorgulamasi yapildi
        ...
        // kullanici direkt fibonacci sayisi girerse calisacak kod satiridir
        }
    }
}